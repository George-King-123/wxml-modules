...
def test_simplest_good_set(q):
  ...
  c = CollectionChecker(m=1, q=q)
  print_test(f"test_simplest_good_set({q})", c.is_good_collection(pts=pts))

def test_simplest_bad_set(q):
  # set of size q + 1 with exactly one duplicate
  pts = [((0, 1),)] * 2 + [((1, d),) for d in range(1, q)]
  c = CollectionChecker(m=1, q=q)
  print_test(f"test_simplest_bad_set({q})", not c.is_good_collection(pts=pts))

def test_quadratic_good_set(q):
  fixed_pt = (0, 1)
  pts = [(p, fixed_pt) for p in get_p1(q)]
  c = CollectionChecker(m=2, q=q)
  print_test(f"test_quadratic_good_set({q})", c.is_good_collection(pts=pts))
...
def test_size_q_fails(q):
  ...
  c = CollectionChecker(m=2, q=q)
  print_test(f"test_size_q_fails({q})", not c.is_good_collection(pts=pts))
