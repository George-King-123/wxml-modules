    ...
    def test_m_1(self):
        for q in PRIMES:
            with self.subTest("m=1 should have one good collection of size q+1",q=q):
                self.assertEqual(investigate_collections(q=q, m=1, size=q+1), 1)
    
    # tests when size = q + 1
    def test_q_2_m_2(self):
        self.assertEqual(investigate_collections(q=2, m=2, size=3), 6)
    
    def test_q_3_m_2(self):
        self.assertEqual(investigate_collections(q=3, m=2, size=4), 8)
